...
## 首頁排版優化（使用者提出的設計回饋，本次已實作）

不是稽核找出來的問題，是使用者看過畫面後主動提出的排版建議（含一份組員給的完整設計回饋文件），本次實作了其中可以安全、有把握做好的部分：

- **播放中收起品牌大標題**：`SMART AMBIENT SOUND / VibeSpace / 讓空間自己找到它的聲音` 這組大標題在按下開始後會收成一行小字（跟 nav bar 的 `VIBESPACE` 字重一致），把版面讓給核心控制項，一般螢幕高度內就能看完不用捲動。用 `.vibe-panel:has(.vibe-orb[aria-pressed="true"])` 這個既有的模式（`.vibe-status`/`.vibe-orb__label` 已經在用）驅動，不需要多寫 JS。
- **主按鈕加上待機呼吸光暈**：原本只有啟動後才有呼吸環動畫，待機時完全沒有互動暗示。新增 `idle-orb-glow`，待機時給一圈很淡的暖色光暈，啟動後自動被原本更明顯的呼吸環動畫接手。已受 `prefers-reduced-motion` 既有規則保護。
- **播放控制群組化**：新增 `.vibe-core` 包住主按鈕、音量指示條、狀態文字、環境狀態框、曲目列（含下一首），縮小彼此間距，減少「散落在頁面上」的感覺；模式切換（Comfort/Balanced/Flow/Manual）獨立成 `.vibe-mode-group`，跟播放控制在視覺上明確分開，因為前者是單次操作，後者是全域風格設定。
  - **踩到一個坑**：把這些元素包進 `.vibe-core` 後，`.vibe-status`/`.vibe-diagnostics`/`.vibe-track` 原本的 `width: min(720px, calc(100% - 32px))` 全部縮水成一兩百 px（因為新的父層沒有明確寬度，`calc(100% - ...)` 的 100% 沒東西可以依據），導致長一點的曲目名稱會不正常換行、按鈕跟著擠壓。修法是幫 `.vibe-core` 補上 `width: 100%`。有實際量過寬度數字（276px → 534px）才發現並修正，不是憑感覺。
- **模式切換改用分段選擇器樣式**：四個模式按鈕從各自獨立的膠囊按鈕，改成共用一個外框的分段控制項，選中的模式有更明顯的底色＋光暈。
- **「查看四種模式的差異」從頁尾文字連結改成 ⓘ 圖示按鈕**：直接放在模式切換列旁邊，不用捲到最下面才找得到說明入口；改動也牽動了 `main.js` 的 toggle 邏輯（原本用 textContent 切換「查看...」/「收起...」文字，現在圖示不需要文字切換，改成切 `.is-active` class，說明文字用靜態 `aria-label` 給螢幕報讀器）。

**沒有做的部分**（回饋裡有提到但這次刻意跳過，需要額外的產品決策或較大工程，先不動）：
- 即時聲學參數（例如「動態分貝範圍」「場域氛圍指數」）——這些目前系統裡沒有計算/顯示過，要做的話得先決定「氛圍指數」實際定義是什麼、用哪個真實數值，不是我可以自己發明的。
- 把音量視覺化（meter bars）整合進主按鈕本身變成聲波動畫——比較大的視覺改版，需要來回調整效果才會好看，這次沒有嘗試。

已用瀏覽器實測待機/啟動兩種狀態、螢幕約 614px 寬（環境限制無法測更窄，但已觸發手機版 CSS 且沒有橫向溢出）、以及展開模式說明表格後的排版，皆正常。`node --test` 全數通過。
